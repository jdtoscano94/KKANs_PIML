...
# %%
data=scipy.io.loadmat(project_root+'/Data/Burger_1000_5000.mat')
print(data.keys())
s_all=data['output']
print(s_all.shape)
x_all=np.linspace(0, 1, s_all.shape[1])
t_all=np.linspace(0, 1, s_all.shape[2])
...
class QR_DeepONet:
    def __init__(self, branch_layers, trunk_layers,X_data,N_fx,output_dim):    
        ...
        self.branch_net = MLP(branch_layers, activation=nn.tanh)
        self.trunk_net = MLP(trunk_layers, activation=nn.tanh) 
