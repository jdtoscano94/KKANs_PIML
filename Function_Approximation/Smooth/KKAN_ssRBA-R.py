...
parser.add_argument('--FEATURES', type=int, default=32, help='Feature size')
...
def get_gradient(apply_fn, params, lambdas, gamma, eta,lamE,lamB, *train_data):
    ...
    r_i=jnp.abs((apply_fn(params, ti, xi) - ui))
    #Function
    loss_fn = lambda params: data_loss(params, ti,xi,ui)
    loss, total_gradient = jax.value_and_grad(loss_fn)(params)
    # Combine loss and gradients
    gradient_flat_inner, _ = flatten_util.ravel_pytree(total_gradient['params']['get_Psi'])
    gradient_flat_outer, _ = flatten_util.ravel_pytree(total_gradient['params']['g_fx'])
